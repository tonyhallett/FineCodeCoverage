...
function logResolveResult(
    logger: PluginLogger,
    resolved: Nullable<ResolveResult>,
    dependency: Dependency
) {
    if (shouldLog) {
        logger.log(
            createDiagnosticWithOrigin(`parent - ${dependency.resolveFrom!}`)
        );

        if (resolved) {
            logger.log(
                createDiagnosticWithOrigin(
                    `resolved ${dependency.specifier} ${JSON.stringify(
                        resolved
                    )}`
                )
            );
...
class ReactAliasingNodeResolver extends NodeResolver {
    ...
    ): Promise<Nullable<NodeResolver.ResolvedAlias>> {
        const resolved =
            filename == reactDom
                ? "react-dom/profiling"
