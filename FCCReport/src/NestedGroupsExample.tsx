...
interface MyGroup
    extends Omit<IGroup, "startIndex" | "count" | "children">,
        MyItem {
    startIndex?: number;
    count?: number;
    children?: MyGroup[];
...
> {
    ...
    private _getGroupNestingLevel(start: number, group: MyGroup): number {
        if (group.children && group.children.length > 0) {
            const childLevels = group.children.map((g) =>
                this._getGroupNestingLevel(start + 1, g)
            );
    ...
    private _onRenderTitle = (props: IGroupHeaderProps | undefined) => {
        ...
        const groupLevel =
            props!.groupLevel === undefined ? 0 : props!.groupLevel;
    ...
    private _onRenderDetailsHeader: any = (
            ...
            <>
                <DetailsList
                    componentRef={(detailsList) =>
                        this.correctScrolling(detailsList)
                    }
                    items={this._items}
