...
export function Coverage(props: CoverageProps) {
    ...
    const onRenderDetailsHeader = useRenderDetailsHeaderSticky(
        active,
        stickyCoverageTable
    );
    ...
    const groups = React.useMemo((): ICoverageGroup[] => {
        switch (grouping) {
            case -1:
                return [
                    new AllGroup(assemblies, namespacedClasses, standalone),
                ];
            case 0:
    ...
    React.useMemo(() => {
        if (groups.length > 1) {
            const rootGroupsSort = columnSort.fieldName
                ? columnSort.fieldName
                : "name";
            const rootGroupAscending = columnSort.fieldName
    ...
    return (
        <>
            ...
            <DetailsList
                ...
                columns={columns}
                getRowAriaLabel={(item: ICoverageItem) =>
                    `${item.name} coverage`
                }
                groupProps={{
                    ...
                    headerProps: {
                        onRenderTitle: (props) => {
                            ...
                            definedProps.expandButtonProps =
                                unsafeExpandButtonProps as any;
                            /* eslint-enable @typescript-eslint/no-explicit-any, @typescript-eslint/no-unsafe-assignment */

                            // groupNestingDepth used for aria
                            const groupLevel =
                                definedProps.groupLevel === undefined
                                    ? 0
                                    : definedProps.groupLevel;

                            const headerGroupNestingDepth =
                                groupNestingDepth - groupLevel - 1;
                    ...
                    onRenderHeader: (props, defaultRender) => {
                        ...
                        const definedDefaultRender =
                            defaultRender as NonNullable<typeof defaultRender>;
                        adjustedColumns.current = definedProps.columns;
                        if (focusingCells) {
                            definedProps.onGroupHeaderKeyUp = (ev) => {
                                const leftOrRightArrow =
                                    ev.code === "ArrowRight" ||
                                    ev.code === "ArrowLeft";
                                if (leftOrRightArrow) {
                                    const groupHeaderRow = (
                                        ev.target as Element
                                    ).closest(`.${groupHeaderRowClassName}`);
                                    if (groupHeaderRow) {
                                        ev.preventDefault();
                                    }
                                }
                ...
                onRenderRow={(rowProps, defaultRender) => {
                    ...
                    const definedDefaultRender = defaultRender as NonNullable<
                        typeof defaultRender
                    >;
