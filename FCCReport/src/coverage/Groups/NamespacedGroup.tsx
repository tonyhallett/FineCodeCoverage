...
export class NamespacedGroup implements ICoverageGroup {
    ...
    constructor(
        ...
        assembly.classes.forEach((cls) => {
            const namespaceGroupingName = this.getGrouping(
                cls.displayName,
                grouping
            );
        ...
        map.forEach((classes, groupingName) => {
            ...
            this.children.push(
                new ClassesGroup(
                    ...
                    groupingName,
                    standalone,
                    `Namespace ${groupingName} class coverage`,
                    1
                )
            );
        });
    ...
    filter(filter: string, hideFullyCovered: boolean): void {
        this.children.forEach((classesGroup) =>
            classesGroup.filter(filter, hideFullyCovered)
        );
    }
    ...
    sort(fieldName: keyof ICoverageItemBase, ascending: boolean) {
        ...
        this.children.forEach((classesGroup) =>
            classesGroup.sort(fieldName, ascending)
        );
    }
}
